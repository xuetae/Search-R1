...
64 training examples and 10 training steps by default, with shorter sequences
and a smaller vLLM cache budget, so the run is sized for two H20 GPUs while
still validating the paper-style method.
